...
- **双登录共存(`dual` profile / `VITE_AUTH_MODE=dual`)**:网关**同时接受两种令牌** —— 本地 his-auth 的 HS256(issuer=his-auth)与 his-idp 的 RS256(issuer=idp),用 `JwtIssuerReactiveAuthenticationManagerResolver` 按 token 的 `iss` claim 分派到对应解码器(`DualResourceServerSecurityConfig`),放行 `/auth/login`;`DualIdentityGlobalFilter` 分流出身份(HS256 直接读 claims、RS256 回查 his-auth),都出 `X-His-*`。前端 dual 模式一页两入口(账号密码 + SSO 按钮),登录方式记 `his_auth_kind`,登出据此分流。纯 password/idp 两模式的 bean 用 `@Profile("!idp & !dual")` 排除,三态互不干扰。启动:`docker compose -f docker-compose.yml -f docker-compose.dual.yml up -d --build`。
...
- **SSO(`idp` profile)专属坑**:① IdP `issuer` 必须**浏览器可达**(默认 `http://localhost:9008`),token/OIDC 元数据据此生成绝对 URL;网关验签用**容器内 DNS** `IDP_JWK_SET_URI=http://his-idp:9008/oauth2/jwks`(两地址不同源,别混用)。② 浏览器 PKCE 换 token 是**跨源 fetch**,his-idp 已对 `/oauth2/**` `/.well-known/**` 开 CORS 放行前端源(`his.idp.web-origins`),改前端端口要同步加。③ 网关是 **WebFlux**,回查 his-auth 用 **reactive `WebClient`(`@LoadBalanced`)**,不是 Feign;`AuthInternalApi` Feign 契约只给 servlet 侧的 his-idp 用。④ IdP RSA 密钥**启动即生成**(重启即轮换,已签发 token 失效)——生产应改为固定密钥。⑤ **网关透传身份 header 必须用 `ServerHttpRequestDecorator` 覆盖 `getHeaders()`**(复制到新的可写 `HttpHeaders`):idp 模式下请求经 Security 过滤链包裹,reactor-netty 头本就是 `ReadOnlyHttpHeaders`,`mutate().headers(consumer)` 和 `mutate().header()` 都会落到只读实例上抛 `UnsupportedOperationException`(HS256 模式无 Security 链,同样写法却能过——真机验证时才暴露)。
- **`dual` 模式(网关双令牌)专属坑**——都是"HS256 与 RS256 token 形态差异",编译期看不出、只有真机跑 HS 链路才炸:① **alg 不是 HS256**:his-auth 用 jjwt `Keys.hmacShaKeyFor` 按密钥字节长自动选 alg,51 字节密钥 → **HS384**;网关 `NimbusReactiveJwtDecoder.withSecretKey` 必须 `macAlgorithm` 与之一致(已按位长 `HS256/384/512` 复刻 jjwt 逻辑),否则验签 401。② **`iss` 非 URL**:HS256 token 的 `iss="his-auth"` 不是合法 URL,分派时不能用 `jwt.getIssuer()`(转 URL 抛 `IllegalArgumentException` → 500),要用 `jwt.getClaimAsString("iss")`。
...
SSO / OIDC(his-idp 授权服务器 + 网关 Resource Server + 前端授权码流,`idp` profile 可选叠加)— ✅ **已真机端到端验证**(容器栈 auth/idp/gateway 重建为 idp 模式,curl 驱动授权码+PKCE:元数据/S256、无token→401、授权码换 token、access_token claims(sub/roles/ADMIN/uid)、id_token 含 realName、admin 建档 200(网关验签→回查角色→X-His-Roles→RBAC)、坏 token→401、cashier 越权→403,9 项断言全过)。password/HS256 主线不受影响(默认 profile)。
双登录共存(`dual` profile:网关同时校验 HS256+RS256 双令牌 + 前端一页两入口)— ✅ **已真机端到端验证**(容器栈 gateway=dual/web=dual,dual_verify.sh:密码 HS256 建档 200 / cashier 403、SSO RS256 建档 200、/auth/login 放行、无/坏 token 401,8 项断言全过)。
